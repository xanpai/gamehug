                ...
                @foreach ($result as $item)
                    @if ($item->reactable->type == 'game' or $item->reactable->type == 'tv')
                        <a class="text-sm flex items-center gap-x-8 text-gray-600 decoration-2 group dark:text-gray-500 mb-5"
                            ...
                            <div class="w-24 aspect-square overflow-hidden relative rounded-md">
                                <img src="{{ $item->reactable->imageurl }}" class="absolute h-full w-full object-cover">
                            </div>
                            <div class="flex-1 dark:text-gray-200">
                                <h3
                                    ...
                                    {{ $item->reactable->title }}</h3>
                                <div class="text-xs text-gray-700 dark:text-white/50 space-x-3 mt-1 flex items-center">

                                    <div class="hidden lg:block">
                                        {{ __('Liked on, :date', ['date' => $item->created_at->diffForHumans()]) }}</div>
                    ...
                    @else
                        <a class="text-sm flex items-center gap-x-8 text-gray-600 decoration-2 group dark:text-gray-500 mb-5"
                            ...
                            <div class="w-24 aspect-square overflow-hidden relative rounded-md">
                                <img src="{{ $item->reactable->post->imageurl }}"
                                    class="absolute h-full w-full object-cover">
                            </div>
                            <div class="flex-1 dark:text-gray-200">
                                <div class="text-xs text-gray-400 before:content-['#']">
                                    {{ __(':number Season', ['number' => $item->reactable->season_number]) . ' ' . __(':number Episode', ['number' => $item->reactable->episode_number]) }}
                                </div>
                                <h3
                                    class="text-base tracking-tighter font-medium text-gray-900 dark:text-gray-300 line-clamp-2 group-hover:underline">
                                    {{ $item->reactable->post->title }}</h3>
                                <div class="text-xs text-gray-700 dark:text-white/50 space-x-3 mt-1 flex items-center">
