                ...
                <div class="flex-1">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3">
                        @foreach ($result['log'] as $item)
                            @if (isset($item->postable->type) and $item->postable->type == 'game' or $item->postable->type == 'tv')
                                <a class="text-sm flex items-center gap-x-8 text-gray-600 decoration-2 group dark:text-gray-500 mb-5"
                                    ...
                                    <div class="w-24 aspect-square overflow-hidden relative rounded-md">
                                        <img src="{{ $item->postable->imageurl }}"
                                            class="absolute h-full w-full object-cover">
                                    </div>
                                    <div class="flex-1 text-gray-900 dark:text-gray-200">
                                        <h3
                                            class="text-base tracking-tighter font-medium text-gray-900 dark:text-gray-300 line-clamp-2 group-hover:underline">
                                            {{ $item->postable->title }}</h3>
                                        <div class="text-xs text-white/50 space-x-3 mt-3 flex items-center">

                                            <div class="text-xxs bg-gray-800 rounded py-0.5 px-1.5 text-gray-300">
                                                {{ $item->postable->type == 'game' ? __('Game') : __('TV Show') }}</div>
                            ...
                            @else
                                <a class="text-sm flex items-center gap-x-8 text-gray-600 decoration-2 group dark:text-gray-500 mb-5"
                                    ...
                                    <div class="w-24 aspect-square overflow-hidden relative rounded-md">
                                        <img src="{{ $item->postable->post->imageurl }}"
                                            class="absolute h-full w-full object-cover">
                                    </div>
                                    <div class="flex-1 text-gray-900 dark:text-gray-200">
                                        <div class="text-xs text-gray-400 before:content-['#']">
                                            {{ __(':number Season', ['number' => $item->postable->season_number]) . ' ' . __(':number Episode', ['number' => $item->postable->episode_number]) }}
                                        </div>
                                        <h3
                                            class="text-base tracking-tighter font-medium text-gray-300 line-clamp-2 group-hover:underline">
                                            {{ $item->postable->post->title }}</h3>
                                        <div class="text-xs text-white/50 space-x-3 mt-3 flex items-center">
                                            <div class="text-xxs bg-gray-800 rounded py-0.5 px-1.5 text-gray-300">
                                                {{ __('Episode') }}</div>
                                        </div>
                ...
                <div class="max-w-sm w-full">
                    <div
                        ...
                        <div class="flex items-center mb-5 space-x-8">
                            <div class="flex-1">
                                <div class="text-xs text-gray-500 dark:text-gray-400">{{ __('Games viewed') }}</div>
                                <div class="text-base font-medium text-gray-700 dark:text-gray-100">
                                    {{ (int) $listing->log_count }}</div>
                            </div>
                        </div>
                        <div class="flex items-center mb-5 space-x-8">
                            <div class="flex-1">
                                <div class="text-xs text-gray-500 dark:text-gray-400">{{ __('Member for') }}</div>
                        ...
                        <div class="flex items-center space-x-8">
                            <div class="flex-1">
                                <div class="text-xs text-gray-500 dark:text-gray-400">{{ __('Content liked') }}</div>
                                <div class="text-base font-medium text-gray-700 dark:text-gray-100">
                                    {{ (int) $listing->like_count }}</div>
                            </div>
                        </div>
                    </div>
                </div>
