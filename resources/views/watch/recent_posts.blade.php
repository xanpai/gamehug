        ...
        <div class="my-6 flex-grow">
            ...
            <div class="bg-gray-900 rounded-lg px-6 py-3 divide-y divide-gray-800/30">
                @forelse ($recentPosts as $date => $posts)
                    <div class="py-4">
                        <h2 class="text-md font-semibold text-gray-300 mb-2">
                        ...
                        @foreach ($posts as $post)
                            <div class="flex group flex-wrap gap-x-6 items-center py-3">
                                <a href="{{ route($post->type, $post->slug) }}" class="flex-1 flex items-center gap-x-6">
                                    ...
                                    <div class="w-28 text-gray-400 text-sm">{{ $post->updated_at->format('H:i') }}</div>
                                    <div
                                        class="text-gray-200 text-sm font-medium dark:text-gray-300 hover:text-gray-900 hover:bg-gray-50/70 hover:border-primary-500 dark:hover:text-gray-100 dark:hover:bg-gray-900 transition rounded-lg">
                                        {{ $post->title }}
                                    </div>
                                </a>
