<div class="relative overflow-hidden">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-24 before:absolute before:-top-14 before:start-1/2 before:bg-[url('../img/hero.svg')] before:opacity-40 before:bg-no-repeat before:bg-top before:w-full before:h-full before:-z-[1] before:transform before:-translate-x-1/2 relative">
        <div class="text-center">
            ...
            <p class="animate-pulse mt-5">Note: All request status will be shared in our Discord server, please join our Discord to view your request status.</p
                </p>
            <div class="mt-7 sm:mt-12 mx-auto max-w-4xl relative">
                <form method="post" action="{{ route('game.request.store') }}" class="flex text-gray-400 relative" @keydown.window.ctrl.q="$refs.searchInput.focus();">
                    @csrf
                    ...
                    <button type="submit" class="absolute right-6 top-1/2 -translate-y-1/2 bg-primary-500 text-white px-4 py-2 rounded-full">
                        {{__('Request')}}
                    </button>
                </form>
            </div>
