                            ...
                            <tr>
                                <td>
                                    <a class="text-sm text-gray-600 dark:text-gray-200 flex items-center space-x-6 group"
                                        ...
                                        <div class="aspect-square rounded-md w-12 overflow-hidden relative">
                                            {!! picture(
                                                $listing->imageurl,
                                                config('attr.poster.size_x') . ',' . config('attr.poster.size_y'),
                                                'absolute h-full w-full object-cover rounded-md',
                                                $listing->name,
                                                'people',
                                            ) !!}
                                        </div>
                                        <div class="">
                                            <div class="font-medium group-hover:underline mb-1">{{ $listing->name }}</div>
                                            @if (!empty($listing->gender))
                                                <div class="text-xs text-gray-400 dark:text-gray-500">
                                                    @if (isset(config('attr.gender')[$listing->gender]))
                                                        {{ config('attr.gender')[$listing->gender] }}
                                                    @else
                                                        {{ $listing->gender }}
                                                    @endif
                                                </div>
