        ...
        <x-form.layout>
            ...
            <form method="post" x-data="{ type: '@if (isset($listing->type)) {{ $listing->type }}@else{{ 'subscribe' }} @endif' }">
                ...
                <div class="mb-4">
                    ...
                    <x-form.input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $listing->name ?? '')"
                ...
                <div class="mb-4">
                    ...
                    <x-form.input id="email" name="email" type="email" class="mt-1 block w-full" :value="old('email', $listing->email ?? '')"
                ...
                <div class="mb-4">
                    ...
                    <x-form.input id="username" name="username" type="text" class="mt-1 block w-full" :value="old('username', $listing->username ?? '')"
                ...
                <div class="mb-4">
                    ...
                    <x-form.select name="account_type">
                        <option value="user" @if (isset($listing->account_type) && $listing->account_type == 'user') selected @endif>{{ __('User') }}
                        </option>
                        <option value="admin" @if (isset($listing->account_type) && $listing->account_type == 'admin') selected @endif>{{ __('Admin') }}
                        </option>
                        <option value="moderator" @if (isset($listing->account_type) && $listing->account_type == 'moderator') selected @endif>{{ __('Moderator') }}
                        </option>
                    </x-form.select>
                ...
                @if (empty($listing->id))
                    <div class="mb-4">
                        ...
                        <div class="mt-3 text-sm text-gray-400 dark:text-gray-300">
                            {{ __('Leave empty if you don\'t want to change it') }}</div>
                    </div>
                @endif
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-x-10">
                    <div class="mb-4 lg:col-span-4">
                        ...
                        <x-form.select name="plan_id">
                            ...
                            @foreach ($plans as $plan)
                                <option value="{{ $plan->id }}" @if (isset($listing->plan_id) and $listing->plan_id == $plan->id ?? null) selected @endif>
                                    {{ $plan->name }}</option>
                            @endforeach
                        </x-form.select>
                    ...
                    <div class="mb-4 lg:col-span-4">
                        ...
                        <x-form.select name="plan_interval">
                            ...
                            <option value="monthly" @if (isset($listing->plan_interval) and $listing->plan_interval == 'monthly') selected @endif>{{ __('Monthly') }}
                            </option>
                            <option value="yearly" @if (isset($listing->plan_interval) and $listing->plan_interval == 'yearly') selected @endif>{{ __('Yearly') }}
                            </option>
                            <option value="prepaid" @if (isset($listing->plan_interval) and $listing->plan_interval == 'prepaid') selected @endif>{{ __('Prepaid') }}
                            </option>
                        </x-form.select>
                        <x-form.error class="mt-2" :messages="$errors->get('plan_interval')" />
                    </div>
                    <div class="mb-4 lg:col-span-4">
                        ...
                        <x-form.input id="plan_ends_at" name="plan_ends_at" type="date" class="mt-1 block w-full"
                            :value="old(
                                'plan_ends_at',
                                isset($listing->plan_ends_at) ? date('Y-m-d', strtotime($listing->plan_ends_at)) : null,
                            )" />
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-12 gap-x-10">
                        <div class="mb-4 lg:col-span-12">
                            ...
                            <x-form.input id="address" name="billing[address]" type="text" class="mt-1 block w-full"
                                :value="old('address') ?? ($listing->billing->address ?? null)" placeholder="{{ __('Address') }}" />
                        ...
                        <div class="mb-4 lg:col-span-4">
                            ...
                            <x-form.input id="city" name="billing[city]" type="text" class="mt-1 block w-full"
                                :value="old('city') ?? ($listing->billing->city ?? null)" placeholder="{{ __('City') }}" />
                        ...
                        <div class="mb-4 lg:col-span-4">
                            ...
                            <x-form.input id="zip_code" name="billing[zip_code]" type="text"
                                class="mt-1 block w-full" :value="old('zip_code') ?? ($listing->billing->zip_code ?? null)" placeholder="{{ __('Postal code') }}" />
