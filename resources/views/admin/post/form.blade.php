...
@section('content')
    <div x-data="post()">
        <form method="post" class="flex space-x-8 2xl:space-x-16" enctype="multipart/form-data">
            <div class="max-w-[90rem] mx-auto w-full" x-data="{ nav: 'overview' }">
                ...
                <div class="mb-5">
                    ...
                    <x-form.textarea name="body" id="editor">{!! old('body', isset($listing) ? $listing->body : '') !!}</x-form.textarea>
            ...
            <div class="max-w-xs w-full">
                ...
                <div class="flex items-center justify-center w-full mb-5">
                    <label for="image-file"
                        ...
                        <div class="flex flex-col items-center justify-center pt-5 pb-6 relative">
                            ...
                            <p class="mb-2 text-sm text-gray-500 dark:text-gray-400 font-medium">
                                {{ __('Click to upload or drag') }}</p>
                            <p class="text-xs text-gray-400 dark:text-gray-400">PNG or JPG (MAX.
                                {{ config('attr.poster.size_x') . 'x' . config('attr.poster.size_y') }})</p>
                        </div>
                        <!-- Show the image -->
                        ...
                        <template x-if="!imageUrl">
                            <div class="object-cover w-full aspect-[2/3] absolute inset-0 rounded-lg"></div>
                        </template>
                ...
                <div class="mb-5">
                    ...
                    <div class="flex items-center space-x-4 mt-2">
                        <x-form.switch type="checkbox" id="featured" name="featured" value="active" :checked="isset($listing) and ($listing->featured == 'active' ? true : false)" />
                        <x-form.label for="featured" class="md:mb-0 cursor-pointer font-normal" :value="__('Show featured')" />
                    </div>
                    <div class="flex items-center space-x-4 mt-2">
                        <x-form.switch type="checkbox" id="slider" name="slider" value="active" :checked="isset($listing) and ($listing->slider == 'active' ? true : false)" />
                        <x-form.label for="slider" class="md:mb-0 cursor-pointer font-normal" :value="__('Show slider')" />
                    </div>
                    <div class="flex items-center space-x-4 mt-2">
                        <x-form.switch type="checkbox" id="member" name="member" value="active" :checked="isset($listing) and ($listing->member == 'active' ? true : false)" />
                        <x-form.label for="member" class="md:mb-0 cursor-pointer font-normal" :value="__('Exclusive to subscriber')" />
                    </div>
                    <div class="flex items-center space-x-4 mt-2">
                        <x-form.switch type="checkbox" id="comment" name="comment" value="active" :checked="isset($listing) and ($listing->comment == 'active' ? true : false)" />
                        <x-form.label for="comment" class="md:mb-0 cursor-pointer font-normal" :value="__('Closed comment')" />
                    </div>
                    <div class="flex items-center space-x-4 mt-2">
                ...
                <x-form.secondary type="button" class="w-full mt-3"
                    @click="importerModal = true;if (importerModal) $nextTick(()=>{$refs.tmdbInput.focus()});">{{ __('Thegamedb importer') }}</x-form.secondary>
            </div>
        </form>
        <div class="fixed inset-0 z-50 flex items-center justify-center overflow-auto bg-gray-900/30 backdrop-blur-lg"
            x-show="bulkModal" x-cloak="">
            <!-- Modal inner -->
            <div class="max-w-3xl w-full px-10 py-10 mx-auto text-left bg-white dark:bg-gray-900 rounded-xl"
                @click.away="bulkModal = false" x-cloak="" x-transition:enter="motion-safe:ease-out duration-300"
                ...
                <div class="mb-5">
                    ...
                    <x-form.textarea x-model="bulkInput" placeholder="{{ __('https://') }}"
                        rows="10"></x-form.textarea>
                    <div class="text-xs text-gray-500 dark:text-gray-400 py-3">
                        {{ __('Each line contains new video link') }}</div>
                </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center overflow-auto bg-gray-900/30 backdrop-blur-lg"
            ...
            <!-- Modal inner -->
            <div class="max-w-lg w-full px-10 py-10 mx-auto text-left bg-white dark:bg-gray-900 rounded-xl"
                @click.away="importerModal = false" x-cloak="" x-transition:enter="motion-safe:ease-out duration-300"
                x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100">
                <!-- Title / Close-->
                <div class="flex items-center justify-between">
                ...
                <form method="post" action="{{ route('admin.tmdb.fetchsingle') }}" @submit.prevent="submitForm()"
                    ...
                    <div class="mb-5">
                        <x-form.label for="advanced" :value="__('Advanced')" />
                        <div class="flex items-center space-x-4 mt-2">
                            <x-form.switch type="checkbox" id="import_people" name="import_people" value="enable"
                                :checked="config('settings.tmdb_people_limit') > 0 ? true : false" />
                            <x-form.label for="import_people" class="md:mb-0 cursor-pointer font-normal"
                                :value="__('Import people')" />
                        </div>
                        @if ($config['nav'] == 'tv')
                            <div class="flex items-center space-x-4 mt-2">
                                <x-form.switch type="checkbox" id="import_season" name="import_season" value="active"
                            ...
                            <div class="flex items-center space-x-4 mt-2">
                                <x-form.switch type="checkbox" id="import_episode" name="import_episode" value="active"
        ...
        <script>
            ...
            function post() {
                return {
                    init() {
                        ...
                        $('.selectize-people').selectize({
                            ...
                            render: {
                                option: function(item, escape) {
                                    ...
                                    return '<div class="flex items-center space-x-8 px-3 py-1">' +
                                        '<div class="aspect-square w-12 bg-cover rounded-md" style="background-image:url(' +
                                        escape(item.image) + '"></div>' +
                                        '<div>' +
                                        (item.name ? '<div class="text-sm font-medium">' + escape(item.name) +
                                            '</div>' : '') +
                                        (item.id ? '<div class="text-gray-400 text-xs">' + escape(item.id) +
                                            '</div>' : '') +
                                        '</div>' +
                                        '</div>';
                                }
                            ...
                            load: function(query, callback) {
                                ...
                                $.ajax({
                                    url: '{{ route('admin.people.search') }}?q=' + encodeURIComponent(
                                        query),
                                    type: 'GET',
                                    dataType: 'json',
                                    error: function() {
                    ...
                    importerData: {!! isset($fetch['data'])
                        ? "JSON.parse('" . addslashes(json_encode($fetch['data'], JSON_UNESCAPED_SLASHES)) . "')"
                        : '[]' !!},
                    peoples: {!! isset($fetch['peoples'])
                        ? "JSON.parse('" . addslashes(json_encode($fetch['peoples'], JSON_UNESCAPED_SLASHES)) . "')"
                        : '[]' !!},
                    tags: {!! isset($listing->tags) ? "JSON.parse('" . addslashes($listing->tags) . "')" : '[]' !!},
                    videos: {!! isset($fetch['videos'])
                        ? "JSON.parse('" . addslashes(json_encode($fetch['videos'], JSON_UNESCAPED_SLASHES)) . "')"
                        : '[]' !!},
                    subtitles: {!! isset($fetch['subtitles'])
                        ? "JSON.parse('" . addslashes(json_encode($fetch['subtitles'], JSON_UNESCAPED_SLASHES)) . "')"
                        : '[]' !!},
                    seasons: {!! isset($fetch['seasons'])
                        ? "JSON.parse('" . addslashes(json_encode($fetch['seasons'], JSON_UNESCAPED_SLASHES)) . "')"
                        : '[]' !!},
                    addVideoField() {
                        this.videos.push({
                            label: '',
                    ...
                    @if ($config['nav'] == 'game')
                        removeVideoField(index, videoId = null) {
                                ...
                                if (videoId) {
                                    ...
                                    $.ajax({
                                        url: '{{ route('admin.' . $config['route'] . '.video.destroy') }}?id=' +
                                            videoId,
                                        type: 'DELETE',
                                        dataType: 'json',
                            ...
                            removeSubField(index, subtitleId = null) {
                                ...
                                if (subtitleId) {
                                    ...
                                    $.ajax({
                                        url: '{{ route('admin.' . $config['route'] . '.subtitle.destroy') }}?id=' +
                                            subtitleId,
                                        type: 'DELETE',
                                        dataType: 'json',
                                        success: function(response) {}
                    ...
                    @if ($config['nav'] == 'tv')
                        removeSeasonField(index, seasonId = null) {
                                ...
                                if (seasonId) {
                                    ...
                                    $.ajax({
                                        url: '{{ route('admin.' . $config['route'] . '.season.destroy') }}?id=' +
                                            seasonId,
                                        type: 'DELETE',
                                        dataType: 'json',
                                        success: function(response) {}
