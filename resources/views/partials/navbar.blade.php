        ...
        <div class="flex items-center relative h-16">
            ...
            @if (empty($hamburger))
                <div class="lg:hidden mr-5">
                    <!-- Hamburger button -->
                    <button class="hamburger text-gray-700 dark:text-gray-100" :class="{ 'active': sidebarToggle }"
                                ...
                                @if ($menu->route)
                                    <li class="@if ($loop->index > 6) {{ 'hidden 2xl:block' }} @endif">
                                        <a href="{{ route($menu->route) }}"
                                            ...
                                            <div class="tracking-tight font-medium whitespace-nowrap flex-1">
                                                {{ __($menu->title) }}</div>
                                        </a>
                                    </li>
                                @elseif($menu->url)
                        ...
                        <li class="relative inline-flex" x-data="{ open: false }">
                            <button class="inline-flex justify-center items-center group" aria-haspopup="true"
                                ...
                                {!! gravatar(
                                    Auth::user()->name,
                                    Auth::user()->avatarurl,
                                    'h-9 w-9 rounded-full bg-primary-500 text-xs font-bold flex items-center justify-center text-white',
                                ) !!}
                            </button>
                            <div class="origin-top-right z-10 absolute inset-x-0 top-full mt-3 bg-white py-6 w-56 px-3 left-auto right-0 lg:-mr-1.5 lg:rounded-xl shadow-lg border border-gray-100 text-sm divide-y divide-gray-100 dark:bg-gray-900 dark:border-gray-900 rtl:lg:mr-0 rtl:left-0 rtl:right-auto"
                    ...
                    @if ($menu->layout == 'all' or $menu->layout == 'header')
                        @if ($menu->route)
                            <a href="{{ route($menu->route) }}"
                                ...
                                <x-ui.icon fill="currentColor" stroke="none" stroke-width="0"
                                    name="{{ $menu->icon }}" class="w-5 h-5"
                                    x-bind:class="compactToggle ? 'w-6 h-6' : ''" />
                                <div class="tracking-tighter whitespace-nowrap flex-1 line-clamp-1"
