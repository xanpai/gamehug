    ...
    @if ($model->comment != 'active' and config('settings.comment') != 'active')
        <div class="flex items-center text-sm gap-x-2 mb-4">
            <div class="relative flex gap-x-3">
                <button
                    class="text-gray-900 dark:text-gray-400 hover:text-gray-200 hover:underline cursor-pointer {{ $orderable === 'id' ? '!text-primary-500' : '' }}"
                    wire:click="orderablex('id')">{{ __('Newest') }}</button>
                <button
                    class="text-gray-900 dark:text-gray-400 hover:text-gray-200 hover:underline cursor-pointer {{ $orderable === 'likes_count' ? '!text-primary-500' : '' }}"
                    wire:click="orderablex('likes_count')">{{ __('Most liked') }}</button>
            </div>
            <div
                class="py-2 flex items-center text-xs text-gray-400 uppercase before:flex-[1_1_0%] before:border-t before:border-gray-200 before:mx-5 dark:text-gray-500 dark:before:border-gray-900 dark:after:border-gray-600 flex-1">
            </div>
